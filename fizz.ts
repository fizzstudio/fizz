...
window.onload = function() {
  let svgroot = document.getElementById("fizz");
...
function update_text( event ) {
  ...
  // }
  var target_el = <HTMLInputElement>event.target;  // Better target?
  if ( target_el ) {
    ...
    console.log( "Target element was null" );
    return;
  }
...
function update_inputs( input_type, text ) {
  var input_el = <HTMLInputElement | null>document.getElementById( input_type + "-input" );
  if ( input_el ) {
    input_el.value = text;
  } else {
    console.log( "Input element with id " + input_type + "-input does not exist" )
    return;
  }
}
...
function update_link( event ) {
  var target_el = <HTMLInputElement>event.target;
  if ( target_el ) {
    var target_id = target_el.id;
    if ( "title-input" == target_id ) {
      let link_target = <HTMLInputElement | null>document.querySelector("input[name='link_target']:checked");
      if ( link_target ) {
        var link_target_value = link_target.value;
        draw.add_link( target_el.value, link_target_value );
      } else {
        console.log( "Link target was null." );
      }
    } else {
      var link_input_el = <HTMLInputElement>document.getElementById( "link-url-input" );
...
function update_link_settings( url_value, link_target_value ) {
  ( <HTMLInputElement>document.getElementById( "link-url-input" ) ).value = url_value;
  ( <HTMLInputElement>document.getElementById( link_target_value ) ).checked = true;
}
...
fizz.prototype.draw_connector = function () {
  ...
  if ( this.active_el ) {
    ...
    if ( !node_1 ) {
      ...
      var start_node = this.attach_node( "start" );      
    }
  }
...
fizz.prototype.draw_text = function () {
  ...
  this.active_el.setAttribute("style", this.get_style( this.default_text_style ));
...
fizz.prototype.handle_inputs = function (event) {
  ...
  this.active_style[prop] = target.value;
  this.deactivate_nodes();
}
...
fizz.prototype.attach_node = function ( node_type ) {
  ...
  // TODO: move this code to tree element manager?
